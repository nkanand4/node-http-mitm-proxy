...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  function openHttpsServer(hostname, callback) {
    var keyFile = path.resolve(self.sslCertCacheDir, hostname + '-key.pem');
    var certFile = path.resolve(self.sslCertCacheDir, hostname + '-cert.pem');
    async.auto({
      ...
      'openPort': function(callback) {
        return openport.find(callback);
      }
    }, function(err, results) {
      if (err) {
        return callback(err);
      }
      console.log('starting server for ' + hostname + ' on port ' + results.openPort);
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  return self._onRequest(ctx, function(err) {
    ...
    // url might have changed, test for it
    if(ctx.proxyToServerRequestOptions.path !== ctx.clientToProxyRequest.url) {
      if(/^\/\//.test(ctx.clientToProxyRequest.url)) {
        ctx.clientToProxyRequest.url = ctx.isSSL ? 'https:':'http:' + ctx.clientToProxyRequest.url;
      }
      if(/^http/.test(ctx.clientToProxyRequest.url)) {
        // if url start with http or https then only update the protocol,
        ctx.isSSL = /^https:/.test(ctx.clientToProxyRequest.url);
      }
      parsedURL = URL.parse(ctx.clientToProxyRequest.url);
      ctx.proxyToServerRequestOptions.host = parsedURL.hostname;
      ctx.proxyToServerRequestOptions.path = parsedURL.path;
      ctx.proxyToServerRequestOptions.port = parsedURL.port;
      ctx.proxyToServerRequestOptions.headers = parsedURL.headers;
    }
  ...
  function makeProxyToServerRequest() {
    ...
    .on( 'error',function ( e ) {
      // handle errors
      console.log('Catching ex', e.code);
    });
